...
class checkbox(Frame):
    def __init__(self, master, label="?"):
        ...
        self.pack()
        self.var = IntVar()
        c = Checkbutton(text=label, variable=self.var, command=self.getValue)
        c.pack(side=LEFT)
...
class SelectRect:
    def __init__(
        ...
        self.nrects = (
            nrects  # max number of rectangles to keep (oldest deleted if too many)
        )
        ...
        if infoboxes:
            for i in range(len(infoboxes)):
                t = self.canvas.create_text(
                    (20, 20 + 30 * i), anchor=SW, text=infoboxes[i], fill="grey"
                )

        if quitLabel:
            Quitter(who=self, quitLabel=quitLabel)  # quit button
    ...
    def ghostFlicker(self):
        ...
        try:
            ...
            self.flicker = 0
        if self.flicker % 2:
            self.canvas.lift(self.imageGhostid)
        else:
    ...
    def putImageGhost(self, x=0, y=0):
        ...
        self.imageGhostid = self.canvas.create_image(
            x, y, anchor=NW, image=self.imgGhost
        )
        self.canvas.lower(self.imageGhostid)
        self.canvas.pack()
    ...
    def markit(self, x, y, symbol=1, r=0):
        ...
        if symbol:
            self.canvas.create_oval(int(x1), int(y1), int(x2), int(y2), tag=MARK)
        else:
            self.canvas.create_rectangle(int(x1), int(y1), int(x2), int(y2), tag=MARK)
    ...
    def polyline(self, pointlist, style=0, tags=[]):
        ...
        if style == 0:
            self.canvas.create_line(ip, fill="red", width=1, tags=tags)
        elif style == 1:
            self.canvas.create_line(ip, fill="green", width=2, tags=tags)
        elif style == 2:
            self.canvas.create_line(ip, fill="blue", width=2, tags=tags)
        elif style == 3:
            self.canvas.create_line(ip, fill="black", width=3, tags=tags)
        elif style == 4:
            self.canvas.create_line(ip, fill="yellow", width=3, tags=tags)
